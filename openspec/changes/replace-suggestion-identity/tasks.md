...
- [x] 2.5 [TEST][REGRESSION] Add Tier 1 compound-v2 matrix. Objetivo: cubrir `stylistic:{type}:{id}`, metadata `compound-v2`, y refs faltantes/corruptas; asegurar que bare ID ya no se soporte. Toca: `src/adapters/word/WordAdapterActionTestHelper.ts`, `src/adapters/word/ApplySuggestionCommand*.test.ts`, `src/adapters/word/WordAdapter*.test.ts`.

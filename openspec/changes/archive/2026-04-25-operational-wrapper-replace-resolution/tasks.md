...
- [x] 2.3 Replace `src/adapters/word/ReplaceIdentityParser.ts` and `src/adapters/word/resolution/SuggestionLocator.ts` with strict wrapper validation/exact lookup that returns explicit `ambiguous-location` or `identity-lost`, never ranked fallback.

## Phase 3: Group resolution architecture

- [x] 3.1 Create `src/adapters/word/resolution/OperationalWrapperGroupResolver.ts` to expand only explicit contiguous/adjacent Stylistic wrapper groups; do not generalize Scenario 22 into a universal distance heuristic.
...
- [x] 4.1 Modify `src/adapters/word/resolution/SuggestionResolutionCleanup.ts` and `src/adapters/word/cleanup/CommentCleanup.ts` to enforce comments-only cleanup after replace resolution and preserve wrapper + inserted-side CC metadata.
...
- [x] 5.4 Update `src/adapters/word/WordAdapterAcceptSuggestion.test.ts`, `WordAdapterRejectSuggestion.test.ts`, `src/taskpane/TaskpaneSuggestionResolution.test.ts`, and related domain tests to assert semantic Word behavior, not permissive implementation trivia.
...
- [x] 6.2 Run focused verification only with standard non-build commands: `npm run typecheck` and `npm run test -- src/adapters/word/WordAdapterAcceptSuggestion.test.ts src/adapters/word/WordAdapterRejectSuggestion.test.ts src/adapters/word/resolution/SuggestionLocator.test.ts src/adapters/word/resolution/SuggestionResolutionObserver.test.ts src/adapters/word/resolution/SuggestionResolutionCleanup.test.ts src/taskpane/TaskpaneSuggestionResolution.test.ts`.
...
- Focused verification passed with `npm run typecheck` and the non-build Vitest suites listed in the apply summary.
